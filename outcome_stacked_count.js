const count_tooltip = d3.select("body")
...
d3.json("./tables/yearly_outcomes_by_race.json").then(data => {
    ...
    const select = d3.select("#outcome-count-select");
    ...
    function drawChart(Outcome) {
        const chartDiv = d3.select("#outcome-count-container");
    ...
    svg.selectAll("g.series")
        ...
        .on("mousemove", (event, d) => {
            const race = d3.select
                (event.currentTarget.parentNode).datum().key;
            const count = d.data[race];
            const percent = ((count / d.data.Total) * 100).toFixed(1);
            count_tooltip.style("left", (event.pageX + 10) + "px")
        ...
        .on("mouseout", () => count_tooltip.style("display", "none"));
    ...
    const legendSvg = d3.select("#outcome-count-legend");
    const padding = 10;
